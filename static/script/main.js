    ...
    setTimeout(() => {
        snake.mv = setInterval(() => {
            ...
            if (gameArray?.[x]?.[y] > 0 || x < 0 || x > settings.width - 1 || y < 0 || y > settings.height - 1) {
                ...
                msg.id = "msg"
                msg.innerHTML = `The end :(<br/>At least the length of your snake was ${snake.len}. Make it bigger!<p id="reset">Press any key to try again...</p>`
                document.getElementById("gameboard").append(msg)
                setTimeout(() => {
